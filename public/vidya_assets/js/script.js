...
$(document).ready(function(){
	...
	// Owl Carousel
	if ($('.product-slide').length > 0 && $.fn.owlCarousel) {
	...
	//Notes Slider
	if ($('.notes-slider').length > 0 && $.fn.owlCarousel) {
	...
	window.initCommonDataTable = function(selector) {
		if (typeof $.fn.DataTable === 'undefined' || typeof $.fn.DataTable.isDataTable !== 'function') {
			return;
		}
		var $tables = $(selector || '.datatable, .dataTable');
		if($tables.length > 0) {
	...
	// Datetimepicker
	if($('.datetimepicker').length > 0 && typeof $.fn.datetimepicker === 'function'){
	...
	if ($('#timer-countdown').length > 0 && $.fn.countdown) {
	...
	if ($('#timer-countup').length > 0 && $.fn.countdown) {
	...
	if ($('#timer-countinbetween').length > 0 && $.fn.countdown) {
	...
	if ($('#timer-countercallback').length > 0 && $.fn.countdown) {
	...
	if ($('#timer-outputpattern').length > 0 && $.fn.countdown) {
	...
	if ($('#summernote2').length > 0 && $.fn.summernote) {
	...
	if ($('#summernote3').length > 0 && $.fn.summernote) {
	...
	if ($('#summernote5').length > 0 && $.fn.summernote) {
	...
	function init() {
		...
		$('.sidebar-menu a').on('click', function(e) {
			...
			if(!$(this).hasClass('subdrop')) {
				...
				$('ul', $(this).parents('ul:first')).not($targetUl).slideUp(250);
				$('a', $(this).parents('ul:first')).not($(this)).removeClass('subdrop');
				$targetUl.slideDown(350);
		...
		try {
			...
			$('.sidebar-menu a').each(function() {
				...
				if (href && href !== '#' && href !== 'javascript:void(0);') {
					try {
						...
						if (linkPath && (currentPath === linkPath || (linkPath.length > 1 && currentPath.indexOf(linkPath) === 0))) {
			...
			if (bestMatch) {
				...
				var $parentSub = bestMatch.parents('li.submenu');
				if ($parentSub.length) {
					var $parentLink = $parentSub.children('a:first');
					$parentLink.addClass('active subdrop');
					$parentSub.children('ul').show();
				}
			} else {
				var $activeSub = $('.sidebar-menu ul li.submenu a.active').parents('li:last').children('a:first');
				if ($activeSub.length && !$activeSub.hasClass('subdrop')) {
					$activeSub.addClass('active subdrop').next('ul').show();
				}
			}
		} catch(err) {
			console.log('Sidebar matcher notice:', err);
		}
	}
	...
	if ($('.bookingrange').length > 0 && $.fn.daterangepicker) {
...
// POS Category Slider
if ($('.video-slide').length > 0 && $.fn.owlCarousel) {
...
function toggleFullscreen(elem) {
  ...
  //Increment Decrement Numberes
	...
	if ($('.timepicker').length > 0 && typeof $.fn.datetimepicker === 'function') {
...
$(document).ready(function(){
	...
	// POS Category Slider
	if ($('.pos-category').length > 0 && $.fn.owlCarousel) {
	...
	if ($('.folders-carousel').length > 0 && $.fn.owlCarousel) {
	...
	// Files Slider
	if ($('.files-carousel').length > 0 && $.fn.owlCarousel) {
	...
	if ($('.video-section').length > 0 && $.fn.owlCarousel) {
	...
	if($('.datanew').length > 0 && typeof $.fn.DataTable === 'function') {
		$('.datanew').DataTable({
			"bFilter": true,
			"ordering": true,
			"language": {
	...
	$(function() {
	 ...
	 if ($('.total-periods-slider').length > 0 && $.fn.owlCarousel) {
	...
	if ($('.time-table-slider').length > 0 && $.fn.owlCarousel) {
	...
	if ($('.routine-slider').length > 0 && $.fn.owlCarousel) {
	...
	if($('.yearpicker').length > 0 && typeof $.fn.datetimepicker === 'function'){
	...
	// Datetimepicker
	if($('.datepic').length > 0 && typeof $.fn.datetimepicker === 'function'){
    ...
    if ($('.inner-pages-slider').length > 0 && $.fn.owlCarousel) {
    ...
    if ($('.teachers-profile-slider').length > 0 && $.fn.owlCarousel) {
	...
	if ($('.link-slider-rtl').length > 0 && $.fn.owlCarousel) {
	...
	if ($('.student-slider').length > 0 && $.fn.owlCarousel) {
	...
	if ($('.student-slider-rtl').length > 0 && $.fn.owlCarousel) {
	...
	if ($('.teacher-slider').length > 0 && $.fn.owlCarousel) {
	...
	if ($('.teacher-slider-rtl').length > 0 && $.fn.owlCarousel) {
	...
	// Lesson
	if ($('.lesson').length > 0 && $.fn.owlCarousel) {
		$('.lesson').owlCarousel({
			items: 3,
			loop: true,
			margin: 15,
	...
	// Task
	if ($('.task-slider').length > 0 && $.fn.owlCarousel) {
