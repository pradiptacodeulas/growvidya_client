...
const RoutineTimetableView = () => {
  ...
  const loadTimetableData = async () => {
    try {
      setLoading(true);
      const [clsRes, secRes, subRes, perRes, daysRes, rtsRes, classTeaRes, shiftsRes] = await Promise.all([
      ...
      const activeShifts = shiftList.filter((s) => s.status === 1);
      setShifts(activeShifts);

      const perList = Array.isArray(perRes?.data) ? perRes.data : Array.isArray(perRes) ? perRes : [];
      const activePeriods = perList.filter((p) => p.status === 1);
      setAllPeriods(activePeriods);

      const shiftPeriods = clsData?.shift_id
        ? activePeriods.filter((p) => Number(p.shift_id) === Number(clsData.shift_id) || !p.shift_id)
        : activePeriods;
      setPeriods(shiftPeriods.length > 0 ? shiftPeriods : activePeriods);
  ...
  // Dynamically compute available periods based on selected shift
  const availablePeriods = useMemo(() => {
    if (!formData.shift_id) {
      return allPeriods;
    }
    const filtered = allPeriods.filter(
      (p) => Number(p.shift_id) === Number(formData.shift_id) || !p.shift_id
    );
    return filtered.length > 0 ? filtered : allPeriods;
  }, [allPeriods, formData.shift_id]);

  const handleShiftChange = (e) => {
    const newShiftId = e.target.value;
    const newAvailable = newShiftId
      ? allPeriods.filter((p) => Number(p.shift_id) === Number(newShiftId) || !p.shift_id)
      : allPeriods;
    const validPeriods = newAvailable.length > 0 ? newAvailable : allPeriods;
    const periodStillValid = validPeriods.some((p) => String(p.id) === String(formData.period_id));

    setFormData((prev) => ({
      ...prev,
      shift_id: newShiftId,
      period_id: periodStillValid ? prev.period_id : (validPeriods[0]?.id ? String(validPeriods[0].id) : ''),
    }));
  };
  ...
  const handleOpenAddModal = (selectedDay = '', selectedPeriod = '') => {
    ...
    const initialDay = selectedDay
      ? String(selectedDay)
      : (days[0]?.id ? String(days[0].id) : '');

    const initialShift = classInfo?.shift_id
      ? String(classInfo.shift_id)
      : (shifts[0]?.id ? String(shifts[0].id) : '');

    const initialPeriod = selectedPeriod
      ? String(selectedPeriod)
      : (periods[0]?.id ? String(periods[0].id) : (allPeriods[0]?.id ? String(allPeriods[0].id) : ''));

    setFormData({
      shift_id: initialShift,
      day: initialDay,
      period_id: initialPeriod,
      subject_id: '',
      teacher_id: '',
    });
    setShowModal(true);
  ...
  const handleOpenEditModal = (routine) => {
    ...
    setFormData({
      shift_id: routine.shift_id
        ? String(routine.shift_id)
        : (classInfo?.shift_id ? String(classInfo.shift_id) : (shifts[0]?.id ? String(shifts[0].id) : '')),
      day: String(routine.day || (days[0]?.id ? String(days[0].id) : '')),
  ...
  const handleSaveRoutine = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        shift_id: formData.shift_id || classInfo?.shift_id || undefined,
      };
  ...
  const routinesByDay = useMemo(() => {
    ...
    routines.forEach((r) => {
      let dKey = String(r.day);
      if (!map[dKey]) {
        const matchedDay = days.find(
          (d) =>
            d.day_name &&
            r.day_name &&
            d.day_name.trim().toLowerCase() === r.day_name.trim().toLowerCase()
        );
        if (matchedDay) {
          dKey = String(matchedDay.id);
        }
      }
        ...
        <div className="card-body p-4">
          {loading ? (
            ...
            <div className="text-center py-5">
              ...
              <p className="text-muted fs-14 fw-semibold mb-3">No working days configured for academic timetable.</p>
            ...
            <div className="d-flex align-items-start overflow-auto pb-3" style={{ minHeight: '400px' }}>
              {days.map((day, dIdx) => {
                ...
                return (
                  ...
                  >
                    {/* Day Heading */}
                    <div className="mb-3 border-bottom pb-2 d-flex align-items-center justify-content-between">
                      <h6 className="fw-bold text-dark mb-0">{day.day_name}</h6>
                      <span className="badge bg-light text-muted border fs-11">
                        {dayList.length} {dayList.length === 1 ? 'Period' : 'Periods'}
                      </span>
                    </div>

                    {/* Period Slots for Day */}
                    {periods.length > 0 ? (
                      periods.map((period, pIdx) => {
                        const routine = dayList.find(
                          (r) =>
                            Number(r.period_id) === Number(period.id) ||
                            (r.period_name && period.period_name && r.period_name.trim().toLowerCase() === period.period_name.trim().toLowerCase())
                        );

                        if (routine) {
                          const themeIndex = (dIdx * 2 + pIdx) % 4;
                          const theme = [
                          ...
                          return (
                            <div
                              key={routine.id || `routine-${period.id}`}
                            ...
                            >
                              ...
                              <div className="d-flex align-items-center justify-content-between mb-1">
                                <span className="d-flex align-items-center text-nowrap text-muted fs-12 fw-medium">
                                  <i className="ti ti-clock me-1 text-primary"></i>
                                  {routine.start_time && routine.end_time
                                    ? `${formatTime(routine.start_time)} - ${formatTime(routine.end_time)}`
                                    : routine.period_name || period.period_name || ''}
                                </span>
                                {routine.shift_name && (
                                  <span className="badge bg-white text-secondary border fs-10 py-1">
                                    {routine.shift_name}
                                  </span>
                                )}
                              </div>
                        ...
                        // Unscheduled slot for this period
                        return (
                          <div
                            key={`empty-${period.id}`}
                            className="border border-dashed rounded p-3 mb-3 bg-white text-center shadow-2xs d-flex flex-column align-items-center justify-content-center"
                            style={{ minHeight: '115px' }}
                          >
                            <p className="d-flex align-items-center justify-content-center text-muted fs-12 fw-medium mb-1">
                              <i className="ti ti-clock me-1 text-primary"></i>
                              {period.period_name} {period.start_time && period.end_time ? `(${formatTime(period.start_time)} - ${formatTime(period.end_time)})` : ''}
                            </p>
                            <span className="badge bg-light text-muted border fs-10 mb-2 py-1 px-2">Not Scheduled</span>
                            <button
                              type="button"
                              className="btn btn-sm btn-outline-primary py-1 px-3 fs-12 d-inline-flex align-items-center gap-1 shadow-2xs"
                              onClick={() => handleOpenAddModal(day.id, period.id)}
                            >
                      ...
                      <div
                        className="border border-dashed rounded p-3 text-center text-muted d-flex flex-column align-items-center justify-content-center mb-3"
                        style={{ minHeight: '115px', backgroundColor: '#fafbfc' }}
                      >
                        <small className="text-muted mb-2">No periods defined for this shift.</small>
                        <Link to="/admin/academics/periods" className="btn btn-sm btn-outline-primary py-1 px-2 fs-12">
                          <i className="ti ti-clock me-1"></i> Add Periods
                        </Link>
                      </div>
                    )}

                    {/* Any extra routines in dayList that don't match configured periods */}
                    {dayList
                      .filter(
                        (r) =>
                          !periods.some(
                            (p) =>
                              Number(p.id) === Number(r.period_id) ||
                              (r.period_name && p.period_name && r.period_name.trim().toLowerCase() === p.period_name.trim().toLowerCase())
                          )
                      )
                      .map((routine, eIdx) => (
                        <div
                          key={routine.id || `extra-${eIdx}`}
                          className="bg-transparent-secondary rounded p-3 mb-3 position-relative border border-secondary-subtle shadow-sm"
                          style={{ backgroundColor: 'rgba(108, 117, 125, 0.08)', minHeight: '115px' }}
                        >
                          <div className="d-flex align-items-center justify-content-between mb-1">
                            <span className="d-flex align-items-center text-nowrap text-muted fs-12 fw-medium">
                              <i className="ti ti-clock me-1 text-secondary"></i>
                              {routine.start_time && routine.end_time
                                ? `${formatTime(routine.start_time)} - ${formatTime(routine.end_time)}`
                                : routine.period_name || ''}
                            </span>
                          ...
                          <div className="bg-white rounded p-1 shadow-sm border border-light d-flex align-items-center justify-content-between">
                            <span className="text-dark fs-12 fw-semibold text-truncate ms-2">
                              {routine.teacher_name || ''}
                            </span>
                            <div className="action-btn d-flex align-items-center gap-2 ms-auto pe-1">
                              <div
                    ...
                    <div className="col-md-6">
                      <div className="mb-3">
                        ...
                        <span className="form-control bg-light text-dark fw-medium" id="shiftSpan">
                          {shifts.find((s) => String(s.id) === String(formData.shift_id))?.shift_name || classInfo?.shift_name || ''}
                        </span>
                        <input type="hidden" name="shift_id" value={formData.shift_id || ''} />
                      </div>
                    </div>
                    ...
                    <div className="col-md-6">
                      <div className="mb-3">
                        ...
                        <span className="form-control bg-light text-dark fw-medium" id="daySpan">
                          {days.find((d) => String(d.id) === String(formData.day))?.day_name || editingRoutine?.day_name || ''}
                        </span>
                    ...
                    <div className="col-md-6">
                      <div className="mb-3">
                        ...
                        <span className="form-control bg-light text-dark fw-medium" id="periodSpan">
                          {(() => {
                            const p = allPeriods.find((item) => String(item.id) === String(formData.period_id));
                            if (p) {
                              return `${p.period_name} ${p.start_time && p.end_time ? `(${formatTime(p.start_time)} - ${formatTime(p.end_time)})` : ''}`;
                            }
                            return editingRoutine?.period_name || '';
                          })()}
                        </span>
                        <input type="hidden" name="period_id" value={formData.period_id || ''} />
                      </div>
                    </div>
