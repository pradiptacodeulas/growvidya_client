...
import adminAcademicApi, { deleteSubjectApi } from '../../../api/adminAcademic.api';
...
const SubjectsList = () => {
  ...
  const filteredSubjects = useMemo(() => {
    let result = subjects.filter((s) => {
      ...
      return (
        ...
        String(s.sort_order || '').includes(q)
      );
    });
    ...
    return result;
  }, [subjects, search, sortField, sortAsc]);
  ...
  const handleExportExcel = () => {
    ...
    let csv = 'Sl No.,Class Name,Subject Name,Sort Order,Status\n';
    subjects.forEach((s, idx) => {
      csv += `"${idx + 1}","${s.class_name || ''}","${s.subject_name || ''}","${s.sort_order || 0}","${s.status === 1 ? 'Active' : 'Inactive'}"\n`;
    });
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `Subjects_List_${new Date().toISOString().split('T')[0]}.csv`;
    a.click();
  ...
  const handleDelete = async () => {
    ...
    try {
      setDeleting(true);
      await deleteSubjectApi(subjectToDelete.id);
      toast.success(`Subject ${subjectToDelete.subject_name} deleted successfully.`);
    ...
    <div className="content">
      ...
      <div className="d-md-flex d-block align-items-center justify-content-between mb-3">
        ...
        <div className="d-flex my-xl-auto right-content align-items-center flex-wrap">
          ...
          <div className="mb-2">
            <Link
              to="/admin/academics/subjects/add"
              className="btn btn-primary d-flex align-items-center"
            >
              <i className="ti ti-square-rounded-plus me-2"></i>Add Subject
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-header d-flex align-items-center justify-content-between flex-wrap pb-0">
          <h4 className="mb-3">All Subject</h4>
        </div>
                        ...
                        currentRecords.map((subject, idx) => {
                          ...
                          return (
                            <tr key={subject.id || idx} className={isEven ? 'even' : 'odd'}>
                              ...
                              <td className="text-center">{subject.class_name || `Class ${subject.class_id || '-'}`}</td>
                              <td className="text-center">{subject.subject_name}</td>
                              <td className="text-center">{subject.sort_order ?? 0}</td>
                                  ...
                                  <div className="dropdown">
                                    ...
                                    <ul className="dropdown-menu dropdown-menu-end p-3">
                                      <li>
                                        <Link
                                          className="dropdown-item rounded-1"
                                          to={`/admin/academics/subjects/edit/${subject.id}`}
                                        >
            ...
            <div className="modal-content">
              ...
              <div className="modal-body text-center py-4">
                ...
                <p className="text-muted mb-0">
                  Do you really want to delete Subject <strong>{subjectToDelete?.subject_name}</strong>? This action cannot be undone.
                </p>
              </div>
