...
const HostelRoomsList = () => {
  ...
  const handleExportExcel = () => {
    ...
    let csv = 'Sl No.,Room Number,Hostel Name,Status\n';
    rooms.forEach((r, idx) => {
      csv += `"${idx + 1}","${r.room_number || r.room_no || ''}","${r.hostel_name || ''}","${Number(r.status) === 1 ? 'Active' : 'Inactive'}"\n`;
    });
  ...
  return (
    <div className="content">
      ...
      <DataTable
        ...
        subtitle="Manage hostel rooms and room allotments."
        columns={columns}
        data={rooms}
        loading={loading}
        selectable={true}
