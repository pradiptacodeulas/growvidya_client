...
const StudentDetails = () => {
  ...
  const getStudentAvatar = (s) => {
    const fallback = (s?.gender === 'Female' || s?.gender_name === 'Female')
      ? femaleUserDefault
      : maleUserDefault;
    return resolveImageUrl(s?.picture, fallback);
  };
  ...
  const getParentAvatarUrl = (pic, defaultGender = 'Male') => {
    const fallback = defaultGender === 'Female' ? femaleUserDefault : maleUserDefault;
    return resolveImageUrl(pic, fallback);
  };
            ...
            <div className="card border-white">
              <div className="card-header">
                <div className="d-flex align-items-center flex-wrap row-gap-3">
                  ...
                  <div className="overflow-hidden">
                    ...
                    <p className="text-primary mb-0">{student.admission_number || 'N/A'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="card-body">
                ...
                <dl className="row mb-0">
                  <dt className="col-6 fw-medium text-dark mb-3">Campus</dt>
                  <dd className="col-6 mb-3 text-dark">{student.branch_name || 'Main Campus'}</dd>
                              ...
                              <div className="d-flex align-items-center mb-3">
                                <span className="avatar avatar-lg flex-shrink-0 me-2">
                                  <img
                                    ...
                                    onError={(e) => {
                                      e.target.onerror = null;
                                      e.target.src = femaleUserDefault;
                                    }}
                                    className="img-fluid rounded"
                                    alt="Mother"
                                    style={{ width: '45px', height: '45px', objectFit: 'cover' }}
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-header">
                <h5 className="modal-title fw-bold">Student Portal Login Credentials</h5>
                <button
                  type="button"
                  className="btn-close"
              ...
              <div className="modal-body">
                <div className="bg-light-300 p-3 rounded border mb-3">
                  <div className="mb-2">
                    <small className="text-muted d-block">Portal Username / Admission No</small>
                    <strong className="text-dark fs-15">{student.admission_number || (student.id ? `STU-${student.id}` : 'N/A')}</strong>
                  </div>
                  <div className="mb-2">
                    <small className="text-muted d-block">Student Name</small>
                    <strong className="text-dark">{student.first_name} {student.last_name || ''}</strong>
                  </div>
                  <div className="mb-2">
                    <small className="text-muted d-block">Student Email</small>
                    <strong className="text-dark">{student.email_address || 'N/A'}</strong>
                  </div>
                  <div className="mb-2">
                    <small className="text-muted d-block">Primary Contact Phone</small>
                    <strong className="text-dark">{student.primary_contact_number || 'N/A'}</strong>
                  </div>
                  <div className="mb-2">
                    <small className="text-muted d-block">Parent / Guardian Contact</small>
                    <strong className="text-dark">{student.parent_phone || student.father_phone || student.mother_phone || 'N/A'}</strong>
                  </div>
                  <div>
                    <small className="text-muted d-block">Account Status</small>
                    <span
                      className={`badge ${
                        Number(student.status) === 1 ? 'bg-success-subtle text-success' : 'bg-danger-subtle text-danger'
                      }`}
                    >
                      {Number(student.status) === 1 ? 'Active & Allowed to Login' : 'Inactive / Suspended'}
                    </span>
                  </div>
                </div>
              </div>
