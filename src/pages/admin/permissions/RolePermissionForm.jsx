...
const RolePermissionForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isReadOnlyRole) {
      toast.error('The Super Admin role is system-protected and cannot be modified.');
      return;
    }
    if (roleName.trim().toLowerCase() === 'super admin') {
      toast.error('The role name "Super Admin" is reserved for the system.');
      return;
    }
    if (!roleName.trim()) {
        ...
        <form onSubmit={handleSubmit} className="d-flex flex-column">
          <div className="card-body p-0 pt-3">
            {loading ? (
              ...
              <>
                {isReadOnlyRole && (
                  <div className="alert alert-warning d-flex align-items-center mx-3 my-2 shadow-sm rounded-3" role="alert">
                    <i className="ti ti-shield-lock fs-24 me-2 text-warning"></i>
                    <div>
                      <strong>Protected System Role:</strong> The Super Admin role has permanent full access to all system modules. Its role name and permission configuration are system-locked and cannot be modified.
                    </div>
                  </div>
                )}
                      ...
                      <label className="ms-3 form-label fw-bold">
                        ...
                        {isReadOnlyRole && (
                          <span className="badge badge-soft-warning ms-2">
                            <i className="ti ti-lock me-1"></i>Protected
                          </span>
                        )}
                      </label>
                      <input
                        type="text"
                        className={`form-control ${isReadOnlyRole ? 'bg-light text-muted' : ''}`}
                        name="role_name"
                        id="role_name"
                                ...
                                <td className="text-center">
                                  <div className="d-flex justify-content-center">
                                    <input
                                      ...
                                      style={{ width: '18px', height: '18px', cursor: isReadOnlyRole ? 'not-allowed' : 'pointer' }}
                                    />
                                  </div>
                                </td>
          ...
          <div className="card-footer bg-white d-flex align-items-center justify-content-end p-3 gap-2">
            <Link to="/admin/roles-permissions" className="btn btn-outline-secondary px-4">
              <i className="ti ti-arrow-left me-1"></i> Back to Roles
            </Link>
            {!isReadOnlyRole && (
              <button
                type="submit"
