...
const RolesList = () => {
  ...
  const handleDeleteClick = (role) => {
    if (isSuperAdminRole(role)) {
      toast.warning('The Super Admin role is system-protected and cannot be deleted.');
      return;
    }
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        sortable: true,
        cell: ({ value, row }) => {
          const isSystem = isSuperAdminRole(row);
          return (
            <div className="d-flex align-items-center">
              ...
              {isSystem && (
                <span className="badge badge-soft-warning me-2 d-inline-flex align-items-center">
                  <i className="ti ti-lock me-1"></i>System Role
                </span>
              )}
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const actionItems = [
            {
              label: isSystem ? 'View Permissions' : 'Edit Permissions',
              icon: isSystem ? 'ti ti-eye text-primary' : 'ti ti-edit-circle text-primary',
              to: `/admin/roles-permissions/edit/${encodeParam(row.id)}`,
            },
          ];
