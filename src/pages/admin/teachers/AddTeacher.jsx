...
const formatImageUrl = (pic) => {
  ...
  if (clean.startsWith('vidya_assets/')) {
    return `${SERVER_BASE_URL}/${clean.replace('vidya_assets/', 'upload/')}`;
  }
...
const AddTeacher = () => {
  ...
  const tabList = [
    { id: 'personal', label: 'Personal Information', icon: 'ti-user' },
    { id: 'address', label: 'Address', icon: 'ti-map-pin' },
    { id: 'class_assign', label: 'Class Assign', icon: 'ti-chalkboard' },
    { id: 'payroll', label: 'Payroll', icon: 'ti-receipt' },
    { id: 'bank', label: 'Bank Account Detail', icon: 'ti-building-bank' },
    { id: 'transport', label: 'Transport Information', icon: 'ti-bus' },
    { id: 'hostel', label: 'Hostel Information', icon: 'ti-building-community' },
    { id: 'social', label: 'Social Media Links', icon: 'ti-brand-share' },
    { id: 'documents', label: 'Documents', icon: 'ti-file-text' },
    { id: 'password', label: 'Password', icon: 'ti-key' },
  ];
  ...
  const defaultBranchId = (activeBranchId && activeBranchId !== 'all' && activeBranchId !== '0') ? activeBranchId : '';
  ...
  const initialClassAssignsRef = useRef([]);
  ...
  // Warning Popup Modal State (for class assign and routine conflicts)
  ...
  const showWarningConfirm = ({ title, message, details, confirmText = 'Proceed', onConfirm }) => {
    setWarningModal({
      show: true,
      title: title || 'Warning: Active Timetable Schedule',
      message,
      details,
      confirmText,
      cancelText: 'Cancel',
  ...
  const loadTeacherForEdit = async (teacherId, opts = {}) => {
    try {
      ...
      setPersonalInfo({
        branch_id: t.branch_id ? String(t.branch_id) : '',
      ...
      if (t.class_assignments && t.class_assignments.length > 0) {
        ...
        const uniqueAssignments = t.class_assignments.filter((ca) => {
          const key = `${ca.class_id}-${ca.subject_id || 0}`;
          if (seenLoaded.has(key)) return false;
          seenLoaded.add(key);
          return true;
        });

        const loadedRows = await Promise.all(
          uniqueAssignments.map(async (ca, idx) => {
        ...
        initialClassAssignsRef.current = loadedRows.map((r) => ({
  ...
  const applyClassAssignClassChange = async (rowIndex, classId) => {
  ...
  const handleClassAssignClassChange = async (rowIndex, classId) => {
    const prevRow = classAssignRows[rowIndex];

    // If editing and changing an already assigned class that has active routines, warn admin
    if (
      isEditMode &&
      prevRow?.class_id &&
      String(prevRow.class_id) !== String(classId)
    ) {
      try {
        const res = await apiClient.get(
          `/admin/academics/routines?teacher_id=${id}&class_id=${prevRow.class_id}`
        );
        const routines = res.data?.data || [];
        if (routines.length > 0) {
          const oldClassName =
            classes.find((c) => String(c.id) === String(prevRow.class_id))?.class_name ||
            'the current class';
          const teacherName =
          ...
          showWarningConfirm({
            title: 'Change Assigned Class?',
            message: `${teacherName} is currently scheduled for ${routines.length} timetable period(s) in ${oldClassName}.`,
            details: `Changing this assigned class will unassign ${teacherName} from those routine periods.`,
            confirmText: 'Change Class',
            onConfirm: () => {
              applyClassAssignClassChange(rowIndex, classId);
            },
          });
          return;
        }
      } catch (err) {
        console.warn('Could not check routine conflicts:', err);
      }
    }
  ...
  const executeRemoveClassAssignRow = (rowIndex) => {
  ...
  const removeClassAssignRow = async (rowIndex) => {
    const targetRow = classAssignRows[rowIndex];

    // If editing and this row has an assigned class, check for active routine entries
    if (isEditMode && targetRow?.class_id) {
      try {
        const res = await apiClient.get(
          `/admin/academics/routines?teacher_id=${id}&class_id=${targetRow.class_id}`
        );
        const routines = res.data?.data || [];
        if (routines.length > 0) {
          const className =
            classes.find((c) => String(c.id) === String(targetRow.class_id))?.class_name ||
            'this class';
          const teacherName =
            `${personalInfo.first_name || ''} ${personalInfo.last_name || ''}`.trim() ||
            'This teacher';
          ...
          showWarningConfirm({
            title: 'Remove Assigned Class?',
            message: `${teacherName} is currently scheduled for ${routines.length} timetable period(s) in ${className}.`,
            details: `Removing this assigned class will unassign ${teacherName} from those routine periods (they will be marked as "No Teacher / Unassigned" so the class timetable remains intact).`,
            confirmText: 'Remove Class',
            onConfirm: () => {
              executeRemoveClassAssignRow(rowIndex);
            },
          });
          return;
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        branch_id: personalInfo.branch_id ? Number(personalInfo.branch_id) : null,
        ...
        class_assignments: (() => {
          const seen = new Set();
          return classAssignRows
            .filter((r) => r.class_id)
            .filter((r) => {
              const key = `${r.class_id}-${r.subject_id || 0}`;
      ...
      if (isEditMode) {
        const executeTeacherSubmit = async () => {
          try {
            setSubmitting(true);
            ...
            toast.error(submitErr.response?.data?.message || 'Failed to update teacher.');
          } finally {
            setSubmitting(false);
          }
        };

        // Safety check: verify if any removed assigned class has active routine slots
        if (initialClassAssignsRef.current.length > 0) {
          const submittedClassIds = new Set(
            (payload.class_assignments || []).map((c) => String(c.class_id))
          );
          const removedClassIds = [
            ...new Set(
              initialClassAssignsRef.current
                .map((c) => c.class_id)
                .filter((cId) => Boolean(cId) && !submittedClassIds.has(cId))
            ),
          ];

          const conflictingClasses = [];
          for (const remClassId of removedClassIds) {
            try {
              const res = await apiClient.get(
                `/admin/academics/routines?teacher_id=${id}&class_id=${remClassId}`
              );
              const routines = res.data?.data || [];
              if (routines.length > 0) {
                const className =
                  classes.find((c) => String(c.id) === String(remClassId))?.class_name ||
                  `Class ${remClassId}`;
                conflictingClasses.push({ className, count: routines.length });
              }
            } catch (checkErr) {
              console.warn('Routine check error:', checkErr);
            }
          }
          ...
          if (conflictingClasses.length > 0) {
            const teacherName =
              `${personalInfo.first_name || ''} ${personalInfo.last_name || ''}`.trim() ||
              'This teacher';
            const classSummary = conflictingClasses
              .map((c) => `${c.className} (${c.count} period(s))`)
              .join(', ');

            setSubmitting(false);
            showWarningConfirm({
              title: 'Confirm Save & Unassign Routine?',
              message: `${teacherName} is currently scheduled in: ${classSummary}.`,
              details: `Saving will unassign ${teacherName} from those routine periods (they will be marked as "No Teacher / Unassigned" so the class timetable remains intact).`,
                    ...
                    <div className="row row-cols-xxl-5 row-cols-md-4">
                      <div className="col-xl-3 col-md-6 mb-3">
                        ...
                        <select
                          ...
                          value={personalInfo.branch_id || ''}
                          onChange={(e) => {
                            setPersonalInfo({ ...personalInfo, branch_id: e.target.value });
                          }}
                        >
                    ...
                    {classAssignRows.map((row, idx) => (
                      <div key={row.id || idx} className="row align-items-center mb-3">
                        ...
                        <div className="col-md-2 d-flex align-items-end mb-2">
                          {(classAssignRows.length > 1 || Boolean(row.class_id)) && (
                            <button
                              type="button"
          ...
          <div className="modal-dialog modal-dialog-centered" style={{ maxWidth: '480px' }}>
            <div className="modal-content border-0 shadow-lg rounded-3">
              ...
              <div className="modal-body text-center pt-0 px-4 pb-4">
                ...
                <h4 className="mb-2 fw-bold text-dark">{warningModal.title}</h4>
                <p className="text-dark fw-medium mb-2" style={{ fontSize: '15px' }}>
                  {warningModal.message}
                </p>
                {warningModal.details && (
                  <p className="text-muted fs-13 mb-4 bg-light p-3 rounded text-start border">
                ...
                <div className="d-flex justify-content-center gap-2">
                  <button
                    ...
                    onClick={() => setWarningModal((prev) => ({ ...prev, show: false }))}
                  >
                    {warningModal.cancelText || 'Cancel'}
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    {warningModal.confirmText || 'Proceed'}
                  </button>
                </div>
              </div>
            </div>
