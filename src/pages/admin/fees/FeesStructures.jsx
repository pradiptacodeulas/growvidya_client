...
const FeesStructures = () => {
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      try {
        const [structRes, classRes, yearRes, compRes, routesRes, hostelsRes, branchRes] = await Promise.all([
        ...
        const hostelsList = Array.isArray(hostelsRes?.data?.hostels)
          ...
          ? hostelsRes.hostels
          : [];
        const branchesList = Array.isArray(branchRes?.data)
          ? branchRes.data
  ...
  const handleOpenAddModal = () => {
    ...
    const defaultBranchId = (activeBranch && activeBranch !== '0' && activeBranch !== 'all') ? activeBranch : '';
  ...
  const handleOpenEditModal = async (struct) => {
    ...
    try {
      ...
      setFormData({
        name: detail.name || '',
        branch_id: detail.branch_id ? String(detail.branch_id) : '',
