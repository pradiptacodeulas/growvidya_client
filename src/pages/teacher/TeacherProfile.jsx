...
import { resolveImageUrl, maleUserDefault } from '../../utils/url.util';
...
const TeacherProfile = () => {
  ...
  const loadProfileAndLocations = useCallback(async () => {
    try {
      ...
      const rawAssignments = t.classAssignments || t.class_assignments || [];
      const seenAssignments = new Set();
      const assignments = rawAssignments.filter((a) => {
        const key = `${a.class_id || a.class_name}-${a.subject_id || a.subject_name || 0}`;
            ...
            <div className="card shadow-sm border-0 mb-0">
              <div className="card-body text-center p-3">
                ...
                >
                  ...
                  >
                    <img
                      src={resolveImageUrl(formData.picture, maleUserDefault)}
                      alt={fullName}
                      style={{
                        width: '100%',
                        height: '100%',
                      ...
                      <div className="d-flex flex-wrap gap-1">
                        {metaInfo.classAssignments.map((ca, idx) => (
                          <span key={ca.id || idx} className="badge bg-light text-dark border fs-11 py-1 px-2">
                            {ca.class_name || ''} {ca.subject_name ? `• ${ca.subject_name}` : ''}
                          </span>
                        ))}
                      </div>
                    </div>
