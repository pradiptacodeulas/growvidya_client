...
const TeacherMyTransport = () => {
  ...
  const getStatusDetails = (statusVal) => {
    ...
    switch (s) {
      ...
      case 2:
        return {
          label: 'Inactive',
          headerLabel: 'Inactive Assignment',
  ...
  const renderStatusBadge = (statusVal) => {
    const details = getStatusDetails(statusVal);
    return (
      <span className={`badge ${details.badgeClass} fw-medium`}>
        <i className={`${details.icon} me-1`}></i>
        {details.label}
      </span>
    );
  };
              ...
              {primaryTransport && (
                <span
                  className={`badge fw-medium px-3 py-2 ${
                    getStatusDetails(primaryTransport.status ?? primaryTransport.staus).badgeClass
                  }`}
                >
                  {getStatusDetails(primaryTransport.status ?? primaryTransport.staus).headerLabel}
                </span>
              )}
            </div>
                    ...
                    {assignedTransports.map((t, idx) => (
                      <tr key={t.id || idx}>
                        ...
                        <td className="text-center">
                          {renderStatusBadge(t.status ?? t.staus)}
                        </td>
                      </tr>
                    ))}
                  </tbody>
