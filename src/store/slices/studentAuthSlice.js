...
import { studentLoginApi, verifyStudentPasscodeApi, getStudentProfileApi, studentLogoutApi } from '../../api/studentAuth.api';
...
export const loginStudentWithPasscode = createAsyncThunk(
  'studentAuth/loginStudentWithPasscode',
  async ({ identifier, passcode }, { rejectWithValue }) => {
    try {
      const response = await verifyStudentPasscodeApi(identifier, passcode);
      const resData = response.data?.data || response.data || {};
      const token = resData.token;
      const student = resData.student;
      ...
      return rejectWithValue(error.response?.data?.message || 'Verification failed. Please check your 6-digit passcode.');
    }
  }
);
...
const studentAuthSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // loginStudentWithPasscode
      .addCase(loginStudentWithPasscode.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(loginStudentWithPasscode.fulfilled, (state, action) => {
        state.loading = false;
        state.isAuthenticated = true;
        state.student = action.payload.student;
        state.error = null;
      })
      .addCase(loginStudentWithPasscode.rejected, (state, action) => {
        state.loading = false;
        state.isAuthenticated = false;
        state.student = null;
        state.error = action.payload;
