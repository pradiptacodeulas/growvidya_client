...
import { teacherLoginApi, verifyTeacherPasscodeApi, getTeacherProfileApi, teacherLogoutApi } from '../../api/teacherAuth.api';
...
export const loginTeacherWithPasscode = createAsyncThunk(
  'teacherAuth/loginTeacherWithPasscode',
  async ({ identifier, passcode }, { rejectWithValue }) => {
    try {
      const response = await verifyTeacherPasscodeApi(identifier, passcode);
      const resData = response.data?.data || response.data || {};
      const token = resData.token;
      const teacher = resData.teacher;
...
const teacherAuthSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Login with Passcode
      .addCase(loginTeacherWithPasscode.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(loginTeacherWithPasscode.fulfilled, (state, action) => {
        state.loading = false;
        state.isAuthenticated = true;
        state.checkingAuth = false;
        state.teacher = action.payload.teacher;
      ...
      .addCase(loginTeacherWithPasscode.rejected, (state, action) => {
        state.loading = false;
        state.isAuthenticated = false;
        state.checkingAuth = false;
        state.error = action.payload;
