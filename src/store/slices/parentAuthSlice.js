...
import { parentLoginApi, verifyParentPasscodeApi, getParentProfileApi, switchActiveStudentApi, parentLogoutApi } from '../../api/parentAuth.api';
...
export const loginParentWithPasscode = createAsyncThunk(
  'parentAuth/loginParentWithPasscode',
  async ({ identifier, passcode }, { rejectWithValue }) => {
    try {
      const response = await verifyParentPasscodeApi(identifier, passcode);
      const resData = response.data?.data || response.data || {};
      const token = resData.token;
      const parent = resData.parent;
...
const parentAuthSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // Login with Passcode
      .addCase(loginParentWithPasscode.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(loginParentWithPasscode.fulfilled, (state, action) => {
        state.loading = false;
        state.isAuthenticated = true;
        state.checkingAuth = false;
        state.parent = action.payload.parent;
      ...
      .addCase(loginParentWithPasscode.rejected, (state, action) => {
        state.loading = false;
        state.isAuthenticated = false;
        state.checkingAuth = false;
        state.error = action.payload;
